...
export function AuthorPage(props: AuthorPageProps) {
    ...
    const fetchData = async (take: number, offset: number) => {
        let req = await fetch(`/api/authors/${id}/books`, {
            method: "POST",
            headers: {
    ...
    useEffect(() => {
        fetchData(take, offset).then(v => {
            setBooks(v.books)
            ;
            setAuName(v.author.name);
            setComment(v.author.comment);
            setCountry(v.author.country);
            setCount(v.count);
            setLoading(false);
        })
    }, [offset,take]);
...
export default connectElem(AuthorPage);
